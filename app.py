...
st.set_page_config(page_title="Mixweave 1.2", page_icon="🎚️", layout="wide")
...
with st.sidebar:
    ...
    genre_rule = st.select_slider(
        "Genre Pockets", options=["Off", "Light", "Normal", "Strong"], value="Normal",
        help="Prefers short 2–4 track runs from the same broad genre family. Three is the sweet spot.",
    )
...
st.caption("Required: Title, Artist, BPM, Camelot Key, Energy  •  Recommended: Genre  •  Optional: Danceability, Valence, Popularity")
...
if not uploaded:
    ...
    with c2:
        st.markdown(
            '<div class="sf-card"><b>📈 Programming Brain</b><p>Shapes broad Energy Zones, spaces artists, and builds useful 2–4 track Genre Pockets.</p></div>',
            unsafe_allow_html=True,
        )
    with c3:
        st.markdown(
...
optional = [c for c in ["Genre", "Danceability", "Valence", "Popularity"] if c in df.columns]
if optional:
    st.caption("Optional metadata detected: " + " • ".join(optional))
if genre_rule != "Off" and "Genre" not in df.columns:
    st.info("Genre Pockets are on, but this playlist has no Genre column. Mixweave will continue without genre programming.")
if invalid_keys.any():
...
if st.button("⚡ Build my Mixweave", type="primary", width="stretch"):
    ...
    if "Genre" in out.columns:
        out["Genre Family"] = [track_genre_family(t) or "Unclassified" for t in ordered]
    ...
    genre = genre_pocket_stats(ordered)
    ...
    c7.metric("Genre islands", genre["islands"] if "Genre" in out.columns else "—")
    ...
    d1.download_button(
        "Download CSV",
        csv_bytes,
        file_name="Mixweave_v1.2_optimized_playlist.csv",
        mime="text/csv",
        width="stretch",
    )
    ...
    d2.download_button(
        "Download Excel",
        xbuf.getvalue(),
        file_name="Mixweave_v1.2_optimized_playlist.xlsx",
